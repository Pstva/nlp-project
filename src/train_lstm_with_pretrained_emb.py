...
class PoetryLSTM(nn.Module):
    def __init__(self, vocab_size, pretrained_emb, hidden_size, num_layers, device, dropout=0.15):
        super().__init__()
        ...
        self.lstm = nn.LSTM(input_size=pretrained_emb.shape[1], hidden_size=hidden_size, num_layers=num_layers, dropout=dropout,
                            batch_first=True)
...
def train_model(vocab, EPOCHS, train_dataloader, emb_input, DEVICE, output_path):
    ...
    model = PoetryLSTM(VOCAB_SIZE, emb_input, HIDDEN_SIZE, NUM_LAYERS, DEVICE).to(DEVICE)
    ...
    for epoch in tqdm(range(EPOCHS)):
        ...
        if (epoch + 1) % 5 == 0:
            save_model(epoch, model, optimizer, loss, f"{output_path}{epoch + 1}ep_300x512x3_bypoem_emb.pth")
...
def main(args):
    ...
    # инициализация предобученных эмбеддингов
    all_words = list(vocab.keys())
    emb_path = args.emb_path #'navec_hudlit_v1_12B_500K_300d_100q.tar'
    ...
    train_model(vocab, int(args.epochs), train_dataloader, emb_input, DEVICE, args.output_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-n", "--emb-path", required=True)
